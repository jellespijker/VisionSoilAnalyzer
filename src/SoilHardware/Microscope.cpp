...
\param frameDelayTrigger the delay between the first initialization of the
microscope and the retrivial of the image expressed in seconds. Default value is
3 seconds
\param dimension A resolution Struct indicating which resolution the webcam
should use. Default is 2592 x 1944
*/
...
void Microscope::StartupSeq(bool firstdefault) {
  ...
  int videodev = find(camNames.begin(), camNames.end(), "USB Microscope") -
                 camNames.begin();
  if (videodev == camNames.size() && !firstdefault) {
    throw Exception::MicroscopeNotFoundException(
        "Microscope Not Found Exception!");
  } else if (videodev == camNames.size() && firstdefault) {
    videodev = 0;
  }
...
/*!< De-constructor*/
Microscope::~Microscope() { captureDevice.~VideoCapture(); }
...
/*! Get an HDR capture of the cam using a user defined number of frames
differently lit frames. Due to hardware limitations each frames take roughly 3
seconds to grab. This function is based upon the tutorial from openCV
...
void Microscope::GetHDRFrame(cv::Mat &dst, uint32_t noframes) {
  ...
  int8_t brightnessStep =
      static_cast<int8_t>((MAX_BRIGHTNESS - MIN_BRIGHTNESS) / noframes);
  ...
  for (uint32_t i = 1; i <= noframes; i++) {
    captureDevice.set(CV_CAP_PROP_BRIGHTNESS,
                      (MIN_BRIGHTNESS + (i * brightnessStep)));
    GetFrame(currentImg);
...
*/
bool Microscope::IsOpened() { return captureDevice.isOpened(); }

/*!< Safely release the capture device*/
void Microscope::Release() { captureDevice.release(); }
