...
namespace Hardware
{
	...
	*/
	string BBB::Read(const string &path)
	{
		ifstream fs;
		fs.open(path.c_str());
		if (!fs.is_open())	{ throw Exception::GPIOReadException(("Can't open: " + path).c_str());	}
		string input;
		getline(fs, input);
		fs.close();
		return input;
	...
	*/
	...
	{
		struct stat st;
		if (stat((char *)path.c_str(), &st) != 0) { return false; }
		return true;
	}
	...
	*/
	bool BBB::CapeLoaded(const string &shield)
	{
		...
		ifstream fs;
		fs.open(SLOTS);
		if (!fs.is_open()) { throw Exception::GPIOReadException("Can't open SLOTS"); }

		string line;
		while (getline(fs, line))
		{
			if (line.find(shield) != string::npos) { shieldFound = true; break; }
		}
		fs.close();
		return shieldFound;
	}
