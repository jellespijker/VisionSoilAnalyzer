...
SoilAnalyzer::AnalysisResults Particle::Analyze(SoilMath::NN &nn) {
  ...
  // Calc the area
  for_each(BW.data, BW.data + (BW.rows * BW.cols),
           [&](uchar P) { Analysis.Area += P; });

  // Calc the edge area
  uint32_t edgeArea = 0;
  for_each(Edge.data, Edge.data + (Edge.rows * Edge.cols),
           [&](uchar P) { edgeArea += P; });

  if (edgeArea >= MIN_EDGE_PIXELS) {
    Analysis.SmallParticle = false;
  } else {
    Analysis.SmallParticle = true;
  }
  ...
  if (!Analysis.SmallParticle) {
    ...
    if (Analysis.Shape.FFT_descriptors.size() >= FFT_DESCRIPTORS) {
      Analysis.Shape.FFT_descriptors.erase(
          Analysis.Shape.FFT_descriptors.begin() + FFT_DESCRIPTORS,
          Analysis.Shape.FFT_descriptors.end());
    } else {
