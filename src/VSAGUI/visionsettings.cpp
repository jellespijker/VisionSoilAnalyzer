...
VisionSettings::VisionSettings(QWidget *parent,
    ...
    : QDialog(parent), ui(new Ui::VisionSettings) {
  ui->setupUi(this);
  if (soilsetting != 0) {
...
VisionSettings::~VisionSettings() { delete ui; }
...
void VisionSettings::on_sb_adaptContrKernel_editingFinished() {
  ...
  if (val % 2 == 0) {
    ui->sb_adaptContrKernel->setValue(val + 1);
  }
...
void VisionSettings::on_sb_adaptContrastFactor_editingFinished() {
  soilSetting->adaptContrastKernelFactor =
      static_cast<float>(ui->sb_adaptContrastFactor->value());
}
...
void VisionSettings::on_sb_morphMask_editingFinished() {
  ...
  if (val % 2 == 0) {
    ui->sb_morphMask->setValue(val + 1);
  }
