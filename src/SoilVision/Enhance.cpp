...
\param kernelsize an uchar which represent the kernelsize should be an uneven
number higher than two
\param factor float which indicates the amount the effect should take place
standard value is 1.0 only used in the adaptive contrast stretch enhancement
\param operation enumerator EnhanceOperation which enhancement should be
performed
*/
Enhance::Enhance(const Mat &src, Mat &dst, uchar kernelsize, float factor,
...
*/
float Enhance::CalculateStdOfNeighboringPixels(uchar *O, int i, int hKsize,
                                               int nCols, int noNeighboursPix,
                                               float mean) {
  uint32_t sum_dev = 0.0;
  float Std = 0.0;
  sum_dev = 0.0;
  Std = 0.0;
  for (int j = -hKsize; j < hKsize; j++) {
    for (int k = -hKsize; k < hKsize; k++) {
      ...
      sum_dev += SoilMath::quick_pow2((O[i + j * nCols + k] - mean));
    }
  }
  // Std = sqrt(sum_dev / noNeighboursPix);
  Std = SoilMath::fastPow((static_cast<double>(sum_dev) / noNeighboursPix), 2);
  return Std;
}
...
*/
void Enhance::CalculateSumOfNeighboringPixels(uchar *O, int i, int hKsize,
                                              int nCols, uint32_t &sum) {
  for (int j = -hKsize; j < hKsize; j++) {
    for (int k = -hKsize; k < hKsize; k++) {
      sum += O[i + j * nCols + k];
    }
...
/*! Homebrew AdaptiveContrastStretch function which calculate the mean and
standard deviation from the neighboring pixels if the current pixel is higher
then the mean the value is incremented with an given factor multiplied with the
standard deviation, and decreased if it's lower then the mean.
\param src cv::Mat source image
\param dst cv::Mat destination image
\param kernelsize an uchar which represent the kernelsize should be an uneven
number higher than two
...
standard value is 1.0 only used in the adaptive contrast stretch enhancement
*/
void Enhance::AdaptiveContrastStretch(const Mat &src, Mat &dst,
                                      uchar kernelsize, float factor) {
  OriginalImg = src;
  ProcessedImg.create(OriginalImg.size(), CV_8UC1);
  AdaptiveContrastStretch(kernelsize, factor);
  dst = ProcessedImg;
...
*/
void Enhance::AdaptiveContrastStretch(uchar kernelsize, float factor,
                                      bool chain) {
  // Exception handling
  EMPTY_CHECK(OriginalImg);
  if (kernelsize < 3 || (kernelsize % 2) == 0) {
    throw Exception::WrongKernelSizeException();
  }
  ...
  while (i++ < pEnd) {
    ...
    float Std = CalculateStdOfNeighboringPixels(O, i, hKsize, nCols,
                                                noNeighboursPix, mean);

    // Stretch

    if (O[i] > mean) {
      // int addValue = O[i] + (int)(round(factor * Std));
      int addValue = O[i] + static_cast<int>(round(factor * Std));
      if (addValue < 255) {
        P[i] = addValue;
      } else {
        P[i] = 255;
      ...
      int subValue = O[i] - static_cast<int>(round(factor * Std));
      if (subValue > 0) {
        P[i] = subValue;
      } else {
        P[i] = 0;
...
void Enhance::HistogramEqualization(bool chain) {
  ...
  if (imgStats.min != imgStats.max) {
    sFact = 255.0f / (imgStats.max - imgStats.min);
  } else {
    sFact = 1.0f;
  }
  ...
  while (i-- > 0) {
    LUT_changeValue[i] = (uchar)(((float)(i)*sFact) + 0.5f);
  }
  ...
  while (i-- > 0) {
    *P++ = LUT_changeValue[*O++ - imgStats.min];
  }
}
}
