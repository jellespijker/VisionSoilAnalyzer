...
*/
void Conversion::Convert(const Mat &src, Mat &dst, ColorSpace convertFrom,
                         ColorSpace convertTo, bool chain) {
  OriginalImg = src;
...
\param chain use the results from the previous operation default value = false;
*/
void Conversion::Convert(ColorSpace convertFrom, ColorSpace convertTo,
                         bool chain) {
  ...
  } else if (convertFrom == CIE_lab && convertTo == RI) // Lab 2 RI
  {
    ...
    throw Exception::ConversionNotSupportedException();
  }
}
...
void Conversion::RGB2Intensity(uchar *O, uchar *P, int nData) {
  ...
  j = 0;
  while (j < nData) {
    P[j++] = (*(O + i + 2) * 0.2126 + *(O + i + 1) * 0.7152 +
              *(O + i) * 0.0722); // Grey value
    i += 3;
  }
}
...
void Conversion::Lab2RI(float *O, float *P, int nData) {
  ...
  for (uint32_t i = 0; i < nData; i += 3) {
    ...
    b = *(O + i + 2);
    P[j++] =
        (L * (pow((pow(a, 2.0f) + pow(b, 2.0f)), 0.5f) * (pow(10, 8.2f)))) /
        (b * pow(L, 6.0f));
  }
}
}
