...
void MorphologicalFilter::Filter(const Mat &mask, bool chain, uchar startVal,
                                 uchar newVal, uchar switchVal) {
  // Exception handling
  CV_Assert(OriginalImg.depth() != sizeof(uchar));
  EMPTY_CHECK(OriginalImg);
  if (mask.cols % 2 == 0 || mask.cols < 3) {
    throw Exception::WrongKernelSizeException("Wrong Kernelsize columns!");
  }
  if (mask.rows % 2 == 0 || mask.rows < 3) {
    throw Exception::WrongKernelSizeException("Wrong Kernelsize rows!");
  }
  ...
  // make Pointers
  Mat workOrigImg(ProcessedImg.rows + mask.rows, ProcessedImg.cols + mask.cols,
                  CV_8UC1);
  workOrigImg.setTo(0);
  if (chain) {
    ProcessedImg.copyTo(workOrigImg(
        cv::Rect(hKsizeCol, hKsizeRow, ProcessedImg.cols, ProcessedImg.rows)));
    // workOrigImg(cv::Rect(hKsizeCol, hKsizeRow, ProcessedImg.cols,
    // ProcessedImg.rows)) = ProcessedImg.clone();
  } else {
    OriginalImg.copyTo(workOrigImg(
        cv::Rect(hKsizeCol, hKsizeRow, ProcessedImg.cols, ProcessedImg.rows)));
    // workOrigImg(cv::Rect(hKsizeCol, hKsizeRow, ProcessedImg.cols,
    // ProcessedImg.rows)) = OriginalImg.clone();
  }
  uchar *O = workOrigImg.data;

  Mat workProcImg(ProcessedImg.rows + mask.rows, ProcessedImg.cols + mask.cols,
                  CV_8UC1);
  ...
  workProcImg.setTo(0);
  if (startVal != 0) {
    workProcImg(cv::Rect(hKsizeCol, hKsizeRow, ProcessedImg.cols,
                         ProcessedImg.rows)).setTo(startVal);
  }
  ...
  ProcessedImg = workProcImg(Rect(hKsizeCol, hKsizeRow, ProcessedImg.cols,
                                  ProcessedImg.rows)).clone();
