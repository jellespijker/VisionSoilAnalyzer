...
/*! MACRO which sets the original pointer to the original image or a clone of
 * the earlier processed image */
#define CHAIN_PROCESS(chain, O, type)                                          \
  if (chain) {                                                                 \
    TempImg = ProcessedImg.clone();                                            \
    O = (type *)TempImg.data;                                                  \
  } else {                                                                     \
    O = (type *)OriginalImg.data;                                              \
  }
/*! MACRO which trows an EmtpyImageException if the matrix is empty*/
#define EMPTY_CHECK(img)                                                       \
  if (img.empty()) {                                                           \
    throw Exception::EmtpyImageException();                                    \
  }
...
public:
  ...
  static Mat CopyMat(const Mat &src, T1 *LUT, int cvType) {
    ...
    if (cvType == 0 || cvType == 8 || cvType == 16 || cvType == 24) {
      for (uint32_t i = 0; i < nData; i += dst.step[1]) {
        dst.data[i] =
            static_cast<uint8_t>(LUT[*(T2 *)(src.data + (i * src.step[1]))]);
      }
    } else if (cvType == 1 || cvType == 9 || cvType == 17 || cvType == 25) {
      for (uint32_t i = 0; i < nData; i += src.step[1]) {
        dst.data[i] =
            static_cast<int8_t>(LUT[*(T2 *)(src.data + (i * src.step[1]))]);
      }
    } else if (cvType == 2 || cvType == 10 || cvType == 18 || cvType == 26) {
      for (uint32_t i = 0; i < nData; i += src.step[1]) {
        dst.data[i] =
            static_cast<uint16_t>(LUT[*(T2 *)(src.data + (i * src.step[1]))]);
      }
    } else if (cvType == 3 || cvType == 11 || cvType == 19 || cvType == 27) {
      for (uint32_t i = 0; i < nData; i += src.step[1]) {
        dst.data[i] =
            static_cast<int16_t>(LUT[*(T2 *)(src.data + (i * src.step[1]))]);
      }
    } else if (cvType == 4 || cvType == 12 || cvType == 20 || cvType == 28) {
      for (uint32_t i = 0; i < nData; i += src.step[1]) {
        dst.data[i] =
            static_cast<int32_t>(LUT[*(T2 *)(src.data + (i * src.step[1]))]);
      }
    }
    return dst;
  }
  ...
  */
  ...
  static Mat CopyMat(const Mat &src, const Mat &mask, int cvType) {
    if (src.size != mask.size) {
      throw Exception::WrongKernelSizeException(
          "Mask not the same size as src Exception!");
    }
    if (mask.channels() != 1) {
      throw Exception::WrongKernelSizeException(
          "Mask has more then 1 channel Exception!");
    }
    Mat dst(src.size(), cvType);
    ...
    for_each(exSrc.begin(), exSrc.end(), [&](const Mat &sItem) {
      ...
      std::transform(sItem.begin<T1>(), sItem.end<T1>(), mask.begin<T1>(),
                     dItem.begin<T1>(),
                     [](const T1 &s, const T1 &m) -> T1 { return s * m; });
  ...
  template <typename T1>
  static void ShowDebugImg(cv::Mat img, T1 maxVal, std::string windowName,
                           bool scale = true) {
    if (img.rows > 0 && img.cols > 0) {
      ...
      if (scale == true) {
        ...
        for_each(exSrc.begin(), exSrc.end(), [&](const cv::Mat &sItem) {
          ...
          std::transform(sItem.begin<T1>(), sItem.end<T1>(), dItem.begin<T1>(),
                         [&](const T1 &s) -> T1 {
