...
\begin{theorem}[The largest ellipse that fits within a square is the inscribed circle]
...
\paragraph{Neurons} The workings of a single neurons is depicted in figure \ref{fig:neuron} and can be described as follows:
\begin{figure}[h]
	\centering
...
Programming of a neural network can be approached in two different ways. Object oriented, where the neurons and layers are classes or imperative oriented where the state of the program chances.
...
\begin{figure}[h]
	\centering
	\begin{tikzpicture}[auto,action/.style={rectangle,draw=ocre!50,fill=ocre!20,thick,inner sep=5pt, minimum size=1cm, rounded corners=5pt}, embeddedaction/.style={rectangle,draw=ocre!50,fill=ocre!10,thick,inner sep=5pt, minimum size=1cm, rounded corners=5pt, dashed},choice/.style={diamond,draw=ocre!50,fill=ocre!20,thick,inner sep=5pt, minimum size=1cm, rounded corners=5pt},combine/.style={circle,draw=ocre!50,fill=ocre!20,thick,inner sep=0pt, minimum size=0.3cm}]

	\node[action] (genesis) {Genesis};
	\node[combine] (evolveloop) [below=of genesis] {};
	\node[action] (crossover) [below=of evolveloop] {Crossover};
	\node[embeddedaction,label=above left:Mutate] (mutate) [below=of crossover]   {\begin{tikzpicture}[action/.style={rectangle,solid,draw=ocre!50,fill=ocre!20,thick,inner sep=5pt, minimum size=1cm, rounded corners=5pt},choice/.style={diamond,solid,draw=ocre!50,fill=ocre!20,thick,inner sep=5pt, minimum size=1cm, rounded corners=5pt},combine/.style={circle,solid,draw=ocre!50,fill=ocre!20,thick,inner sep=0pt, minimum size=0.3cm},every node/.style={scale=0.6}]
		\node[combine] (start) {};
