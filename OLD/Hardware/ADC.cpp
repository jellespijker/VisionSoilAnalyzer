...
namespace Hardware
{
	...
	*/
	int ADC::GetCurrentValue()
	{
		...
		Intensity = (float)(retVal - MinIntensity)/(4096 - (MinIntensity + (4096 - MaxIntensity)));
		return retVal;
	}
	...
	\param callback the function which should be called when polling indicates a change CallbackType 
	\return 0
	*/
	int ADC::WaitForValueChange(CallbackType callback)
	{
		...
		if (pthread_create(&thread, NULL, &threadedPollADC, static_cast<void*>(this)))
		{
			threadRunning = false;
	...
	*/
	int ADC::WaitForValueChange()
	{
		...
		if ((fd = open(adcpath.c_str(), O_RDONLY | O_NONBLOCK)) == -1) { throw Exception::ADCReadException();	}
		ev.events = EPOLLIN;
		ev.data.fd = fd;

		if (epoll_ctl(epollfd, EPOLL_CTL_ADD, fd, &ev) == -1) {	throw Exception::FailedToCreateGPIOPollingThreadException("ADC: Failed to add control interface!");	}
